...
const AddProfileFormScreen = ({route, navigation }: any) => {
  const { addProfile } = useProfiles();
  const currentUserPhone =  route.params?.loginUser ?? '';
  ...
  const handleSubmit = async () => {
    ...
    // You can send this profile to backend or context here
    const profile = {
      ...
      aboutMe,
      manglik,
      createdByUserPhone: currentUserPhone,
      
    };
    try {
      const response = await axios.post('http://10.0.2.2:3000/api/add/addprofile', profile);

      addProfile(profile);

      Alert.alert(`Success, Profile has been added successfully! with message status ${response.status}`);
      navigation.goBack();
    } catch (error) {
    ...
    // TODO: Save profile to backend or context here
  ...
  return (
    <View>
      ...
      <ScrollView style={styles.container}>
        ...
        <View style={styles.formGroup}>
          ...
          <View style={styles.pickerContainer}>
            ...
            >
              ...
              <Picker.Item label="Vyas" value="Vyas" />
            </Picker>
          </View>
        </View>
        ...
        </View>        <View style={styles.formGroup}><Text style={styles.label}>Village / Locality</Text><TextInput style={styles.input} value={village} onChangeText={setVillage} placeholder="Enter village/locality" /></View>
        ...
        <View style={styles.formGroup}>
          ...
          <View style={styles.pickerContainer}>
            <Picker
              ...
              onValueChange={(itemValue) => setManglik(itemValue)}
            >
              <Picker.Item label="Select Manglik Status" value="" />
